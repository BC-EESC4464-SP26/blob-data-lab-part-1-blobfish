...
% looking at figure of moving std dev, the values plateau around
% october2013 and the peaks of the plateous are around 0.3 so we used that 
% a cutoff based on date would be ideal to completely exclude the noisy
% data


%6b. Find the indices of the data points that you are not excluding based
%on the cutoff chosen in 6a.
cleaned_movestd = NaN(length(movestd),1);
clean_index = find(movestd <= 0.3);
cleaned_movestd(clean_index) = movestd(clean_index);
